...
class RestaurantController extends Controller {
    ...
    public function actionView($id) {
		$MenuMaster = MenuMaster::find()->all();
        $MenuMaster = ArrayHelper::map($MenuMaster, 'id', 'title');
        $DishMaster = DishMaster::find()->all();
        $DishMaster = ArrayHelper::map($DishMaster, 'id', 'title');
        ...
        try {
            if ($model->load(Yii::$app->request->post())) {
                ...
                if ($model->save()) {
					...
					if ($imageAPP!== false) {
						$destinationPathApp = Yii::$app->params['logoPath'] . "resize/" . $model->db_app_logo;
						CommonHelper::resize($pathAPP, $destinationWidth = 150, $destinationHeight = 100, $destinationPathApp);
					}
                    if ($imageF !== false) {
...
//        return $this->render('create', ['model' => $model]);
    ...
    public function actionUpdate($id) {
		...
		$oldAPPFile = $model->getAPPImageFile();
        $oldAPPRFile = $model->getResizeAPPImageFile();
		
        $oldFIFile = $model->getImageFileImage();
        $oldFIRFile = $model->getResizeImageFileImage();
        $oldAPPAvatar = $model->db_app_logo;
        ...
        if ($model->load(Yii::$app->request->post())) {
			...
			if($imageAPP=== false){
				$model->db_app_logo =$oldAPPAvatar;
			}
            ...
            if ($model->save()) {
				...
				if ($imageAPP !== false) {  // delete old and overwrite
                    if (is_file($oldAPPFile)) {
                        unlink($oldAPPFile);
                    }
                    if (is_file($oldAPPRFile)) {
                        unlink($oldAPPRFile);
                    }
                    $pathAPP = $model->getAPPImageFile();
                    $imageAPP->saveAs($pathAPP);
                    $destinationPathApp = Yii::$app->params['logoPath'] . "resize/" . $model->db_app_logo;
                    CommonHelper::resize($pathAPP, $destinationWidth = 100, $destinationHeight = 100, $destinationPathApp);
                }
