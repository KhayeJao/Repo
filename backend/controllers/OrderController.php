...
class OrderController extends Controller {
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create', 'update', 'delete', 'placeorder', 'selectdelivery','rejectmsg', 'adddish', 'refreshcart', 'removedish', 'emptycart', 'checkout', 'checkoutpricess', 'addcomment', 'selectaddress', 'changestatus', 'outputinfo','neworder','newordershow','orderviewstaus', 'orderchangestatus','manuallyassign'],
                        'roles' => ['@']
                    ]
                ]
            ]
    ...
    public function actionCheckoutpricess() {
        ...
        if($post_date['Order']['order_to']=='restaurant'){
			...
			if($user_type=="L"){ 
				...
				$order_model->order_unique_id = $this->getRandomOrderID()."_C"; 
			}
        ...
        if ($post_date['Order']['user_id']) { 
			...
			if($user_type=="L" || $user_type=="callcenter"){  
				
				 $address_model = AddressL::findOne(['id' => yii::$app->request->post('address_id')]); 
                		 
				 $order_model->address_line_1 = $post_date['Order']['address_line_1'];
				 $order_model->address_line_2 = $post_date['Order']['address_line_2'];
				 $order_model->area = $post_date['Order']['area'] ;
				 $order_model->city = $post_date['Order']['city'] ;
				 $order_model->pincode = $post_date['Order']['pincode'] ;
				 
				 
			 } 
			 
             if($user_type=="callcenter"){
				$user_from = 'callcenter';
			 }else{
				$user_from = 'telecaller'; 
			 }
			//echo $user_from,"<pre/>";print_r($post_date['Order']);die;
			/* insert on logistic member table */
			 ...
			 if(!$L_user['email']){
				 ...
				 $connection->createCommand()->insert('tbl_logistic_user', ['restaurant_id' => $restaurant_ids,'first_name'=>$first_name,'last_name'=>$last_name,'mobile_no' => $post_date['Order']['mobile'],'email' => $post_date['Order']['email'],'user_to' => $post_date['Order']['order_to'],'type' =>  'guest','user_from'=>$user_from,'created_at' => strtotime($date),'updated_at' =>  strtotime($date),])->execute();
    ...
    public function actionSelectaddress() {
        ...
        if ($_POST['delivery_type'] == 'Delivery') {
            $_SESSION['cart']['delivery_charge'] = $delivery_charges;
            $response = array(
                'status' => 1,
	...
	public function actionSelectdelivery() {
        ...
        $response = array(
            'status' => 1,
            'message' => 'Delivery charge set to zero'
        );
        return Json::encode($response);
    }
	...
	/* send reject msg */
	 ...
	 public function actionRejectmsg(){
	    $post_date = yii::$app->request->post(); 
		$model = Order::findOne(['id' => $post_date['order_id']]); //1063
					$status_reason = $post_date['msg']; 
                    if ($post_date['status'] == 'Rejected') {
						$model->order_status_change_reason = $status_reason; 
                        $model->order_status_change_datetime = date('Y-m-d H:i:s', time());
                        $model->accept_reject_datetime = date('Y-m-d H:i:s', time());
						$model->status = "Rejected";
                        if ($status_reason == "Other") {
                            $model->order_status_change_reason = $post_date['cmsg'];
                        }
							$model->save(FALSE);
							$response = array(
							'status' => 1,
    ...
    private function  sendpushmessage($order_id=0,$db_id=0){
		...
		// echo "lat ".$latitude ."long ".$longitude;
		 ...
		 $query = "SELECT device_id , "
                    ...
                    . "where is_active = 1 and is_available=1 and "  
                    . "ROUND((" . $unit . " * 3956 * acos( cos( radians('$latitude') ) * "
                    . "cos( radians(latitude) ) * "
                    . "cos( radians(longitude) - radians('$longitude') ) + "
                    . "sin( radians('$latitude') ) * "
...
/* for ios 
  ...
  public function actionOrderchangestatus($id='', $act='') {
    ...
    private function sendOrderEmails($id) {
	  ...
	  if($placed_via!='Restaurant/Admin'){
		  
        /** EMAIL TO ADMIN * */
		 
		if(strpos($order->order_unique_id,'_C')==false){
			//echo strpos($order->order_unique_id,'_C');die;
		 ...
		 if(yii::$app->user->identity->type=='admin' || yii::$app->user->identity->type=='restaurant' || yii::$app->user->identity->type=='telecaller'){
					      ...
					      if(yii::$app->user->identity->type=='restaurant'){ 
					          ...
					          $order_model_today = \common\models\base\Order::findAll(['status' => 'Approved','close' => '0','placed_via'=>'Telecaller/Admin','Date(accept_reject_datetime)'=>$date]);
					           $order_model_today_pace = $connection->createCommand("SELECT * FROM tbl_order where status='Approved' && close='0' && restaurant_id='".$restaurant_model->id."' && placed_via='Restaurant/Admin' &&  Date(booking_time)='".$date."' ")->queryAll(); 
						  }else{
	  ...
	  public function actionNewordershow(){
		   ...
		   if(yii::$app->user->identity->type=='admin' || yii::$app->user->identity->type=='restaurant' || yii::$app->user->identity->type=='telecaller'){
					      ...
					      if(yii::$app->user->identity->type=='restaurant'){
							  ...
							  $order_model_today_pace = $connection->createCommand("SELECT * FROM tbl_order where status='Approved' && close='0' && restaurant_id='".$restaurant_model->id."' && placed_via='Restaurant/Admin' &&  Date(booking_time)='".$date."' ")->queryAll(); 
						      //echo "SELECT * FROM tbl_order where status='Approved' && close='0' && placed_via='Restaurant/Admin' &&  Date(booking_time)='".$date."' ";
                   				 //print_r($order_model_today_pace);die;
						 }else{
							  $order_model = \common\models\base\Order::findAll(['status' => 'Placed','close' => '0','placed_via'=>'Telecaller/Admin']); 
		 ...
		 if(yii::$app->user->identity->type=='restaurant'){
			  foreach($order_model as $order_val){ 
				  if($i<4){
					   ...
					   $date      = date('Y-m-d H:i:s'); 
					   $interval  = abs(strtotime($date)-strtotime($order_val['booking_time']));
					   $minutes   = round($interval / 60);
					  
					   if($order_val['placed_via']=='Telecaller/Admin' && $minutes>5 && $order_val['view']==0 && $order_val['close']==0 && strpos($order_val['order_unique_id'],'_C')!=false ){
						    //echo $minutes;
						    $ht='<li><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_val->id.'"><i class="fa fa-envelope"></i> New order by '.$order_val->user_full_name.' is placed <span>'.$newDateTime.'</span> </a></li>';
							echo "
							<script type=\"text/javascript\">
							 var htm = '".$ht."';
							 
							$('#approveModal').modal('show');
							$('#approveModalData').html(htm);
							</script>
							";
					   ...
					   $html.='<li class="sp'.$order_val->id.'"><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_val->id.'"><i class="fa fa-envelope"></i> New order by '.$order_val->user_full_name.' is placed <span>'.$newDateTime.'</span> </a><a style="color:#'.$color.'" href="#"> <span class="sp" id="'.$order_val->id.'">X</span></a></li>';
					/*  //save notification start  // 
							 $noti_model = new Notifications;
							 $noti_model->resturant_id= $order_val['resturant_id'];
							 $noti_model->user_id = $order_val->user_id;
							 $noti_model->masage  = $html;
							 $noti_model->created_at  = strtotime(date('Y-m-d H:i:s'));
							 $noti_model->save(false);
					  // end // */
				 }
				 $i++;
			  } 
		...
		if(yii::$app->user->identity->type=='telecaller' or yii::$app->user->identity->type=='restaurant'){
									...
									if($i<4){
										 ...
										 if($order_model_todays->status=="Approved"){
										  ...
										  /* for send notification to telecoller  when order assign to delivery boy  autometicaly by nearby */
											...
											if($order_model_todays->placed_via!='Restaurant/Admin'){
												...
												if($minutes<2){
													$html.='<li><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_model_todays->id.'"><i class="fa fa-envelope"></i> Order from '.ucfirst($order_model_todays->user_full_name).'  is accepted by '.ucfirst($restaurant->title).' <span>'.$newDateTime.'</span> </a><a style="color:#'.$color.'" href="#"><span class="sp" id="'.$order_model_todays->id.'">X</span> </a></li>';
												     $data ='<a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_model_todays->id.'"><i class="fa fa-envelope"></i> Order from '.ucfirst($order_model_todays->user_full_name).'  is accepted by '.ucfirst($restaurant->title).' <span>'.$newDateTime.'</span> </a><a style="color:#'.$color.'" href="#"><span class="sp" id="'.$order_model_todays->id.'">X</span> </a>';
										 ...
										 if($deliveryBoyOrder){
												 ...
												 if($deliveryBoyOrder->status =="Acknowledge"){ 
                                                   ...
                                                   /*   //save notification start  // 
													...
													if($order_model_todays->placed_via!='Restaurant/Admin'){
														$html.='<li><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_model_todays->id.'"><i class="fa fa-envelope"></i> Order from  '.ucfirst($restaurant->title).'  has not yet been assigned for Delivery 
								...
								foreach($order_model_today_pace as $order_model_today_paces){ 
									...
									if($i<5){
                                         ...
                                         $data = $connection->createCommand("SELECT  * FROM tbl_order where id='".$order_model_today_paces['id']."' AND booking_time >= NOW( ) - INTERVAL 5 MINUTE ")->queryAll();
											...
											if($minutes>=5 ){
												if($order_model_today_paces['status']=='Approved' && $order_model_today_paces['status']=='Telecaller/Admin'){
												...
												/*//save notification start  // 
													$noti_model = new Notifications;
													$noti_model->resturant_id= $order_model_today_paces['resturant_id'];
													$noti_model->user_id = $order_model_today_paces['user_id'];
													$noti_model->masage  =  $html;
													$noti_model->created_at  = strtotime(date('Y-m-d H:i:s'));
													$noti_model->save();
													// end //
										 ...
										 $restaurant = \common\models\base\Restaurant::findOne(['id' => $order_model_today_paces['restaurant_id']]);
										 $color=($order_model_today_paces['view']==0 ? "808080" :"A4A4A4");
										 if($order_model_today_paces['status']=="Approved"){
										 
										 // echo $order_model_today_paces['restaurant_id'];
										    
										 
										  /* for send notification to telecoller  when order assign to delivery boy  autometicaly by nearby */
								            $deliveryBoyOrder = \common\models\base\DeliveryBoyOrder::findOne(['order_id' => $order_model_today_paces['id']]);
											 $date = date('Y-m-d H:i:s'); 
											 $interval  = abs(strtotime($date)-strtotime($order_model_today_paces['booking_time']));
											 $minutes   = round($interval / 60); 
											 
											if($order_model_today_paces['placed_via']!='Restaurant/Admin'){
												if($minutes<2){
													$html.='<li><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_model_today_paces['id'].'"><i class="fa fa-envelope"></i> Order from '.ucfirst($order_model_todays->user_full_name).'  is accepted by '.ucfirst($restaurant->title).' <span>'.$newDateTime.'</span> </a><a style="color:#'.$color.'" href="#"><span class="sp" id="'.$order_model_today_paces['id'].'">X</span> </a></li>';
												}
											}
										   //echo $order_model_todays->placed_via ;die;
										 if($deliveryBoyOrder){
												 ...
												 if($deliveryBoyOrder->status =="Acknowledge"){ 
													...
													// $minutes =  (time()-strtotime($deliveryBoyOrder->created_at))/60*1000; 
													 ...
													 if($minutes>2){
														...
														$html.='<li><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_model_today_paces['id'].'"><i class="fa fa-envelope"></i> Delivery boy&#39; '.ucfirst($user_info->first_name." ". $user_info->last_name).'  has been assigned the delivery of '.ucfirst($restaurant->title).' <span>'.$newDateTime.'</span> </a><a style="color:#'.$color.'" href="#"><span class="sp" id="'.$order_model_today_paces['id'].'">X</span> </a></li>'; 
														
													}else{
														
													  $html.='<li><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_model_today_paces['id'].'"><i class="fa fa-envelope"></i>  '.ucfirst($user_info->first_name." ". $user_info->last_name).'  has not yet acknowledged the order from '.ucfirst($restaurant->title).' <span>'.$newDateTime.'</span> </a><a style="color:#'.$color.'" href="#"><span class="sp" id="'.$order_model_today_paces['id'].'">X</span> </a></li>'; 
													...
													// echo "d",$deliveryBoyOrder->status;die;
													  $html.='<li><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_model_today_paces['id'].'"><i class="fa fa-envelope"></i>  '.ucfirst($user_info->first_name." ". $user_info->last_name).'  is ready to pick up the order of  '.ucfirst($restaurant->title).' <span>'.$newDateTime.'</span> </a><a style="color:#'.$color.'" href="#"><span class="sp" id="'.$order_model_today_paces['id'].'">X</span> </a></li>';
													  
												 }elseif($deliveryBoyOrder->status =="Pickup"){
													 
													  $html.='<li><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_model_today_paces['id'].'"><i class="fa fa-envelope"></i>  '.ucfirst($user_info->first_name." ". $user_info->last_name).'  has picked up the order 0f  '.ucfirst($restaurant->title).' <span>'.$newDateTime.'</span> </a><a style="color:#'.$color.'" href="#"><span class="sp" id="'.$order_model_today_paces['id'].'">X</span> </a></li>';
												   ...
												   //echo $order_model_today_paces['placed_via'];die;
													if($order_model_today_paces['placed_via']='Restaurant/Admin' || $order_model_today_paces['placed_via']='Telecaller/Admin'){
														if($minutes>5){
														 
													$html.='<li><a style="color:#'.$color.'" href="'.Yii::$app->request->baseUrl.'/order/view?id='.$order_model_today_paces['id'].'"><i class="fa fa-envelope"></i> Order from  '.ucfirst($restaurant->title).'  has not yet been assigned for Delivery 
												<span>'.$newDateTime.'</span> </a><a style="color:#'.$color.'" href="#"><span class="sp" id="'.$order_model_today_paces['id'].'">X</span> </a></li>'; 
														}
													}
											}
