...
<div class="order-view">
    <div class="col-md-12 back-col">
        ...
        <div class="form-group col-md-4 add-textp">
            
            <div >
                <label> <?= $restaurant_model->title ?> (<?= $restaurant_model->food_type ?>) </label>
            </div>
             
                
                <div >
                    <?= $restaurant_model->address ?>, <?= $restaurant_model->area ?>,  <?= $restaurant_model->city ?>
                </div>
            ...
            <div >
                <i class="fa fa-clock-o"></i> <?= $restaurant_model->open_datetime_1 ?> - <?= $restaurant_model->close_datetime_1 ?></br>
                <i class="fa fa-clock-o"></i> <?= $restaurant_model->open_datetime_2 ?> - <?= $restaurant_model->close_datetime_2 ?>
            </div>
        </div>
        ...
        <div class="form-group col-md-8"> 
			...
			foreach ($restaurant_delivery_areas as $key => $value) {
				array_push($areas_arr, trim($value->area->area_name));
			}
			?>
                 <?= (count($areas_arr) > 0 ? (substr(implode(', ', $areas_arr),0,110).' <a href="#areaModal" data-toggle="modal" data-target="#areaModal">View More</a>') : 'Not Specified'); ?> <i class="fa fa-map-marker"></i> 
            </div>
        <?php 
		$restaurant_cuisines = $model->restaurantCuisines; 
		$cuisines_arr = array(); 
		...
		?>
            <div class="min-back">
            <ul class="min-order">
				<?php if(count($cuisines_arr)>0){ ?>
		        <li> <i class="fa fa-spoon"></i><i class="fa fa-spoon"></i> <a href="#cuisinesModal" data-toggle="modal" data-target="#cuisinesModal">
                  <?= ($cuisines_arr ? (substr(implode(', ', $cuisines_arr),0,15).' <i class="fa fa-share-square"></i>') : ' - '); ?>
                  </a>
                </li>
                <?php }?>
			<li><i class="fa fa-inr"></i> <?= $restaurant_model->min_amount ?> ( Min. Amount ) | </li>
            <li> Avg. Rating : <?= $restaurant_model->avg_rating ?></li>
            </ul>
            </div>
        </div>
    ...
    <div class="col-md-12 nopadding"> 
      <div class="col-md-8 nopadding">
        <h3 class="red-col">Menu Info <?= $_SESSION['cart']['table_no']?></h3>
      ...
      <div class="col-md-4 col-sm-12 col-xs-12" id="order_container">
            <div>
                <h2 class="list-view-fake-header" id="cart_icon"><i class="fa fa-shopping-cart"></i> Shopping Cart <a href="javascript:void(0);" title="Empty Cart" class="empty_cart pull-right"><span class="fa fa-trash fa-2x"></span></a></h2>
               
            </div>

            <?php if (trim($restaurant_model->coupon_text)) { ?>
                <div class = "col-md-12 get-disc">
                   <span><i class="percentage-percentage"></i></span> <?= $restaurant_model->coupon_text ?>
                </div>
            <?php } else if ($restaurant_model->restaurantCoupons) { ?>
                <div class = "col-md-12  get-disc">
                   <span><i class="percentage-percentage"></i></span>  <?php
  ...
  <div class="modal-dialog"> 
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title areasm">Delivery areas</h4>
      </div>
      <div class="modal-body areamodel" style="float:left; width:100%;">
        <div class="row" style="width:31%;margin-right:3%; float:left;">
          <?php
			
			$countlieee = 0;
			$countli = count($areas_arr);
			$divclose = round($countli/3) ;
			sort($areas_arr);
			for($i=0;$i<count($areas_arr);$i++){
			$countlieee++;
			?>
          <div class="col-sm-12 gridlist"><span><?php echo $areas_arr[$i]; ?></span></div>
          <?php 

			if ($countlieee%$divclose==0) echo '</div> <div class="row" style="width:31%;margin-right:3%; float:left;">';
...
<div id="cuisinesModal" class="modal fade" role="dialog">
  <div class="modal-dialog"> 
...
<?php $this->registerCss(".table.table-condensed thead tr th, .table.table-condensed tbody tr td, .table.table-condensed tbody tr td *{white-space:normal} .sub-tr td{border-bottom : none !important; padding:0 20px !important;} .gridlist span{
	line-height:30px;
	border-bottom:1px solid #CCC;
	padding-bottom:10px;
} .modal-header {
    ...
    padding: 15px;
    text-align: center;
} #finale_print{display:none}.table.table-condensed thead tr th, .table.table-condensed tbody tr td, .table.table-condensed tbody tr td *{white-space:normal} .sub-tr td{border-bottom : none !important; padding:0 20px !important;} .areasm {
    color: #fff;
}"); ?>
