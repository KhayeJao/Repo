...
use common\models\LogisticUser;

/**
 * UserSearch represents the model behind the search form about `common\models\base\User`.
 */
class LogisticCallcenterUserSearch extends LogisticUser {
...
// bypass scenarios() implementation in the parent class
    ...
    public function search($params) {
		 ...
		 $res_id           = $restaurant_model->id; 
         $query            = LogisticUser::find();
        //$resturanIds  = LogisticUser::getRestaurantID();
        $query->andFilterWhere(['like', 'restaurant_id', $res_id]);
		 $query->andFilterWhere(['like', 'user_from', 'callcenter']);
...
// $query->where('0=1');
        ...
        if ((\yii::$app->user->can('viewUserList')) AND ( !\yii::$app->user->can('manageUsers')) AND ( !\Yii::$app->user->can('placeOrder' ))){ // TELECALLER ACCESS
            $query->andFilterWhere(['like', 'type', 'guest']);
        }
